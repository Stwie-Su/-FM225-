...
def video_loop(cap):
    global video_cap, latest_frame, video_frame_count, video_last_frame_time, video_width, video_height
    while video_running:
        frame = None
        if cap is not None:
            ok, f = cap.read()
            if ok and f is not None:
                ...
                video_height, video_width = f.shape[:2]
        if frame is None:
...
def start_video(device, maxv, auto=True):
    ...
    global video_frame_count, video_start_time, video_last_frame_time, video_width, video_height
...
def api_video_snapshot():
    ...
    return Response(buf.tobytes(), mimetype='image/jpeg',
                    headers={'Content-Disposition': f'attachment; filename="fm225_snapshot_{ts}.jpg"'})
...
def api_status():
    fps = 0.0
    if video_running and video_start_time > 0:
        elapsed = time.time() - video_start_time
        if elapsed > 0:
            fps = round(video_frame_count / elapsed, 1)
    real = False
    if HAVE_CV2 and video_cap is not None:
        try:
            real = video_cap.isOpened()
        except Exception:
            pass
    return jsonify({'have_cv2': HAVE_CV2, 'video_running': video_running,
                    ...
                    'video_fps': fps, 'video_frame_count': video_frame_count,
                    'video_real': real,
